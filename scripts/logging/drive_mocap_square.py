#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Drive a square using mocap PoseStamped feedback."""
...
def apply_config(args, argv):
    ...
    square = config.get("control", {}).get("square", {})
    values = {
        "pose_topic": mocap.get("pose_topic"),
        "cmd_topic": mocap.get("cmd_topic"),
        "line_yaw_offset_deg": mocap.get("line_yaw_offset_deg"),
        "side_length": square.get("side_length"),
        "linear": square.get("linear"),
        "min_linear": square.get("min_linear"),
        "heading_kp": square.get("heading_kp"),
        "turn_heading_kp": square.get("turn_heading_kp"),
        "max_angular": square.get("max_angular"),
        "turn_max_angular": square.get("turn_max_angular"),
        "min_turn_angular": square.get("min_turn_angular"),
        "turn_slowdown_angle_deg": square.get("turn_slowdown_angle_deg"),
        "slowdown_distance": square.get("slowdown_distance"),
        "position_tolerance": square.get("position_tolerance"),
        "turn_tolerance_deg": square.get("turn_tolerance_deg"),
        "max_lateral_error": square.get("max_lateral_error"),
        "pose_timeout": square.get("pose_timeout"),
    }
...
def pose_age():
    return time.time() - last_pose_wall_time
...
def run_leg(pub, args, leg_index, line_yaw, target_rigid_yaw):
    ...
    while not rospy.is_shutdown() and not stop_requested:
        ...
        if args.verbose and now - last_report >= args.report_period:
            print(
                "leg %d: t=%.1fs along=%.3f/%.3f lateral=%.3f heading=%.1fdeg cmd=(%.3f, %.3f, %.3f)"
                % (
                    leg_index + 1,
                    elapsed,
                    along,
...
def run_square(pub, args):
    ...
    print(
        "Mocap square: topic=%s frame=%s start=(%.3f, %.3f, %.3f) rigid_yaw=%.1fdeg line_yaw=%.1fdeg side=%.3fm linear=%.3f dry_run=%s"
        % (
            args.pose_topic,
            start_pose[4],
            start_pose[0],
...
def parse_args(argv):
    parser = argparse.ArgumentParser(description="Drive a mocap-feedback square")
    ...
    parser.add_argument("--legs", type=int, default=4,
                        help="Number of square legs to run. Use 2 for first-corner testing.")
    parser.add_argument("--linear", type=float, default=0.07)
...
def main(argv):
    ...
    rospy.init_node("drive_mocap_square", anonymous=True)
